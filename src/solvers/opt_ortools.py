...
class MaxCutCallback(cp_model.CpSolverSolutionCallback):
    ...
    def on_solution_callback(self):
        ...
        if obj > self.best_obj:
            ...
            self.logger.debug(f"New best cut: {int(obj)}.  Partition sizes: |A|={len(part0)}, |B|={len(part1)}")
...
def ortools_solve_maxcut(mygraph: nx.Graph, nworkers: int = 16, timeout: int = 3600) -> MaxCutCallback:
    """
    ...
    Returns:
       MaxCutCallback: Partition assignment of nodes.
    """
    ...
    logger.debug(f"Time to solve the problem: {round(t_duration, 4)} sec")  
    
    callback.status = status
    callback.runtime = t_duration    

    if status == cp_model.OPTIMAL:
        logger.debug(f"Optimal value: {solver.ObjectiveValue()}")
        return callback
    elif status == cp_model.FEASIBLE:
        logger.debug(f"Feasible solution found with value: {solver.ObjectiveValue()}")
        return callback
    else:
